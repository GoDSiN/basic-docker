...
server.mount_proc '/health' do |req, res|
    res.status = 200
    res.body = {'status' => 'Test is healthy'}.to_json
...
server.mount_proc '/test' do |req, res|
    ...
    begin
        ...
        test = get_book_test(id, headers)
        res.body = test.to_json
        res['Content-Type'] = 'application/json'
    rescue => error
...
# TODO: provide test on different books.
def get_book_test(id, headers)
    if ENV['ENABLE_EXTERNAL_BOOK_SERVICE'] === 'true' then
      ...
      # that has Shakespeare as the single author
        isbn = '0486424618'
        return fetch_test_from_external_service(isbn, id, headers)
    end
...
def fetch_test_from_external_service(isbn, id, headers)
    uri = URI.parse('https://www.googleapis.com/books/v1/volumes?q=isbn:' + isbn)
    http = Net::HTTP.new(uri.host, ENV['DO_NOT_ENCRYPT'] === 'true' ? 80:443)
    http.read_timeout = 5 # seconds

    # DO_NOT_ENCRYPT is used to configure the test service to use either
